...
%this.metaData.trialTypes=trialTypes;%Carly added this
...
for i=1:length(trialsInCond)%CJS
    if isempty(trialsInCond{i})
        break
    else
        if length(trialTypes)>100
            CondType{i}=trialTypes{trialsInCond{i}(1)+100};
        else
            CondType{i}=trialTypes{trialsInCond{i}(1)};
        end
    end
end
...
for itype=1:length(types)
    ...
    if isempty(refConditions)
        [baseCond]=this.metaData.getConditionsThatMatch('base',types{itype}, CondType);
    else
    ...
    if ~isempty(baseCond)
        switch upper(types{itype})
            ...
            otherwise %'TM' and any other
                ...
                removeData=zeros(size(newData));%CJS: Solution Line 1
                removeData(inds, :)=data;%CJS: Solution Line 2
                newData(isnan(removeData))=nan;%CJS: This is the orgin of the problem
                base(base==0)=nan;
