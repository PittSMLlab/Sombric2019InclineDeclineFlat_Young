...
%See also: labDate
    ...
    methods
        ...
        function [condNames]=getConditionsThatMatch(this,name,type, trialTypes)
           ...
           if nargin>2 && ~isempty(type) && isa(type,'char')
               if exist('trialTypes') %CJS added
                    typeMatches=cellfun(@(x) ~isempty(x),(strfind(lower(trialTypes),lower(type))));%orignal
               else
               typeMatches=cellfun(@(x) ~isempty(x),(strfind(lower(this.conditionName),lower(type))));%orignal
               end
           else
               typeMatches=true(size(patternMatches));
           end
           condNames=this.conditionName(patternMatches(1:length(typeMatches)) & typeMatches);%cjs
           if length(condNames)>1 %CJS THIS WAS IMPLIMENTED 2/13
               if sum(strcmp(lower(condNames), 'tm base'))>=1 
                   condNames=condNames(strcmp(lower(condNames), 'tm base'));
               elseif sum(strcmp(lower(condNames), 'og base'))>=1 
                   condNames=condNames(strcmp(lower(condNames), 'og base'));
               else
                   error('Couldnt determine which baseline file to use to remove bias.')
               end
           end
        end
    end
